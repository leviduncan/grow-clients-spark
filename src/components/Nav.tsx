...
export default function Nav() {
  ...
  return (
    <>
      {/* Glass over the page while the mobile sheet is open.

          A SIBLING of <header>, not a child. The header carries z-50 on a
          positioned element, which creates a stacking context: nested
          inside it, z-40 would be resolved against that context and the
          overlay would paint above the bar's own contents rather than
          beneath them.

          inset-0 covers the whole viewport. The bar and the sheet are both
          opaque bg-band at z-50, so the glass only actually reads below
          them, which is the intent.

          Tinted with `band` rather than the always-dark `scrim` token, so
          it follows the theme: a white wash on the light ground, a deep
          slate one in dark. Same blur/saturate recipe as the scrolled bar.

          md:hidden mirrors the sheet's own guard. `open` survives a resize
          to desktop, and without this the overlay would strand over a page
          whose sheet had already been hidden.

          onClick closes, because an overlay that swallows a tap and does
          nothing is worse than no overlay. Escape and the X button remain
          the accessible routes, so this stays aria-hidden. */}
      {open && (
        <div
          aria-hidden="true"
          onClick={() => setOpen(false)}
          className="fixed inset-0 z-40 bg-band/60 backdrop-blur-xl backdrop-saturate-150 md:hidden"
        />
      )}
